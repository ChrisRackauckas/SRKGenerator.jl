...
function integral_function(x)
  a2, a3, b13, b23, A031, B021, B031, B032 = x
  b22 = -1 - b23
  A021 = 1
  c01 = 0
  c03 = 1
...
x0 = rand(8)
opt = Opt(:LN_COBYLA, 8) #:LD_SLSQP, :LN_COBYLA (semi), :GN_ISRES, :LN_AUGLAG_EQ support equality constraints
lower_bounds!(opt, lbs)
upper_bounds!(opt, ubs)
max_objective!(opt, f)
equality_constraint!(opt::Opt,constraints!, [1e-16 for i in 1:3])
ftol_abs!(opt,1e-16)
xtol_rel!(opt,1e-16)
...
c02 = A021
G(A021,A031,A032,a1,a2,a3)
g_res = zeros(3); g_res_grad = zeros(14,3); c_res=zeros(3)
